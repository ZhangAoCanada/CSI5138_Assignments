...
class WGAN:
    ...
    def Discriminator(self, input_x, reuse=False):
        with tf.variable_scope("discriminator") as scope:
            ...
            count = 0
            layer = self.ConvBlock(input_x, 64, count=count)
            count += 1
    ...
    def Generator(self, input_z, reuse=False):
        with tf.variable_scope("generator") as scope:
            ...
            count += 1
            for _ in range(self.num_hidden_layers):
                layer = self.DeconvBlock(layer, self.hidden_size // 2, count=count)
                count += 1
            layer = self.DeconvBlock(layer, self.hidden_size // 4, count=count, stride=2)
            count += 1
            for _ in range(self.num_hidden_layers):
                layer = self.DeconvBlock(layer, self.hidden_size // 4, count=count)
                count += 1       
